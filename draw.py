...
def scanline_convert(polygons, i, screen, zbuffer):
	...
	if polygons[i][1] >= polygons[i+1][1] and polygons[i][1] >= polygons[i+2][1]:
		...
		t = polygons[i+1]
	elif polygons[i+2][1] >= polygons[i][1] and polygons[i+2][1] >= polygons[i+1][1]:
		t = polygons[i+2]
	...
	if t == polygons[i]:
		if polygons[i+1][1] >= polygons[i+2][1]:
			m = polygons[i+1]
			b = polygons[i+2]
		elif polygons[i+2][1] >= polygons[i+1][1]:
			m = polygons[i+2]
			b = polygons[i+1]
	elif t == polygons[i+1]:
		if polygons[i][1] >= polygons[i+2][1]:
			m = polygons[i]
			b = polygons[i+2]
		elif polygons[i+2][1] >= polygons[i][1]:
			m = polygons[i+2]
			b = polygons[i]
	elif t == polygons[i+2]:
		if polygons[i][1] >= polygons[i+1][1]:
			m = polygons[i]
			b = polygons[i+1]
		elif polygons[i+1][1] >= polygons[i][1]:
			m = polygons[i+1]
			b = polygons[i]
	...
	dx1_1 = (t[0] - m[0]) / (t[1] - m[1] + 1)
	dz0 = (t[2] - b[2]) / (t[1] - b[1] + 1)
	dz1 = (m[2] - b[2]) / (m[1] - b[1] + 1)
	dz1_1 = (t[2] - m[2]) / (t[1] - m[1] + 1)
